...
* It minimizes Steinhart-Hart residuals in a sane way. The residuals appear to be random, symmetric, and are comparable with the measurement error (cold junction instability, reference voltage accuracy, and rounding errors in digital displays). This result and the non-convexity of probe deviation may be aspects of the same phenomenon; if they are, it is still nice to be a able to observe it by a couple different methods.
