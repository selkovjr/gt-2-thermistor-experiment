...
data.beta_2 <- read.table('hotend-thermistor-error.tab')
...
names(data.beta_2) = c('setting', 'probe')
...
ggplot() +
  ...
  geom_point(data = data.beta_2, mapping = aes(x = probe, y = setting - probe, color = 'c. zero gradient'), shape = 19, size = 1.5) +
  # geom_line(data = data.beta_2, mapping = aes(x = probe, y = setting - probe, color = 'c. zero gradient'), size = 0.5) +
  geom_point(data = data.proxy_1, mapping = aes(x = probe, y = setting - probe, color = 'd. proxy 1'), shape = 19, size = 1.5) +
  # stat_smooth(data = data.proxy_1, mapping = aes(x = probe, y = setting - probe, color = 'd. proxy 1'), method = "loess", size = 0.5) +
  geom_point(data = data.proxy_2, mapping = aes(x = probe, y = setting - probe, color = 'e. proxy 2'), shape = 19, size = 1.5) +
  # stat_smooth(data = data.proxy_2, mapping = aes(x = probe, y = setting - probe, color = 'e. proxy 2'), method = "loess", size = 0.5) +
  xlab(expression('Probe temperature, ' * degree * C)) +
  ylab(expression(italic(T)[setpoint] - italic(T)[probe] * ', ' * degree * C)) +
  theme(legend.position = c(0.025, 0.95), legend.justification=c(0,1), legend.text.align = 0, legend.title = element_blank()) +
